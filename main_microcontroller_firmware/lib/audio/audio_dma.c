...
// the number of stalls we can tolerate when the SD card takes longer to write than usual
// With 10 chunks at 8192 samples each @ 384kHz, this gives ~213ms of stall tolerance per channel
#define DMA_NUM_STALLS_ALLOWED (10)
...
#endif
#if (AUDIO_DMA_BUFF_LEN_IN_BYTES % AUDIO_DMA_MAIN_BUFFER_LEN_MANDATORY_LCM)
#error "Audio DMA buffer length in bytes must be divisible by 2, 4, 8, and 16"
#endif
...
/*
 * DMA buffer chunks spread across multiple SRAM regions to utilize all available memory.
 * Each chunk is AUDIO_DMA_BUFF_LEN_IN_BYTES (24KB) and must be contiguous, but chunks
 * can be in different SRAM regions since DMA only needs contiguity within a single transfer.
 *
 * MAX32650 SRAM layout:
 *   SRAM0-3:  0x20000000 - 0x2003FFFF (256KB) - used for stack, heap, general vars
 *   SRAM4-5:  0x20040000 - 0x2007FFFF (256KB) - .sram45 section
 *   SRAM6-7:  0x20080000 - 0x200FFFFF (512KB) - .sram6 section (256KB workspace + 256KB for DMA)
 *
 * With 10 stalls allowed per channel (20 chunks total @ 24KB each = 480KB), we spread them:
 *   - CH0 chunks 0-4 in SRAM6-7 (120KB)
 *   - CH0 chunks 5-9 in SRAM4-5 (120KB)
 *   - CH1 chunks 0-4 in SRAM6-7 (120KB)
 *   - CH1 chunks 5-9 in SRAM4-5 (120KB)
 */

// Channel 0 chunks - first 5 in SRAM6-7, next 5 in SRAM4-5
__attribute__((section(".sram6"))) static uint8_t dma_chunk_ch0_0[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram6"))) static uint8_t dma_chunk_ch0_1[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram6"))) static uint8_t dma_chunk_ch0_2[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram6"))) static uint8_t dma_chunk_ch0_3[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram6"))) static uint8_t dma_chunk_ch0_4[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram45"))) static uint8_t dma_chunk_ch0_5[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram45"))) static uint8_t dma_chunk_ch0_6[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram45"))) static uint8_t dma_chunk_ch0_7[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram45"))) static uint8_t dma_chunk_ch0_8[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram45"))) static uint8_t dma_chunk_ch0_9[AUDIO_DMA_BUFF_LEN_IN_BYTES];

// Channel 1 chunks - first 5 in SRAM6-7, next 5 in SRAM4-5
__attribute__((section(".sram6"))) static uint8_t dma_chunk_ch1_0[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram6"))) static uint8_t dma_chunk_ch1_1[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram6"))) static uint8_t dma_chunk_ch1_2[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram6"))) static uint8_t dma_chunk_ch1_3[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram6"))) static uint8_t dma_chunk_ch1_4[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram45"))) static uint8_t dma_chunk_ch1_5[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram45"))) static uint8_t dma_chunk_ch1_6[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram45"))) static uint8_t dma_chunk_ch1_7[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram45"))) static uint8_t dma_chunk_ch1_8[AUDIO_DMA_BUFF_LEN_IN_BYTES];
__attribute__((section(".sram45"))) static uint8_t dma_chunk_ch1_9[AUDIO_DMA_BUFF_LEN_IN_BYTES];

// Pointer arrays for easy indexed access to chunks
static uint8_t *dma_chunks_ch0[DMA_NUM_STALLS_ALLOWED] = {
    dma_chunk_ch0_0, dma_chunk_ch0_1, dma_chunk_ch0_2, dma_chunk_ch0_3, dma_chunk_ch0_4,
    dma_chunk_ch0_5, dma_chunk_ch0_6, dma_chunk_ch0_7, dma_chunk_ch0_8, dma_chunk_ch0_9
};
static uint8_t *dma_chunks_ch1[DMA_NUM_STALLS_ALLOWED] = {
    dma_chunk_ch1_0, dma_chunk_ch1_1, dma_chunk_ch1_2, dma_chunk_ch1_3, dma_chunk_ch1_4,
    dma_chunk_ch1_5, dma_chunk_ch1_6, dma_chunk_ch1_7, dma_chunk_ch1_8, dma_chunk_ch1_9
};
...
int audio_dma_init()
{
    ...
    if ((res = MXC_DMA_Init()) != E_NO_ERROR)
    {
        return res;
    }

    dma_channel_0_idx = MXC_DMA_AcquireChannel();
    dma_channel_1_idx = MXC_DMA_AcquireChannel();
    ...
    ch0_config_reg = &MXC_DMA->ch[dma_channel_0_idx].cfg;
    ch0_status_reg = &MXC_DMA->ch[dma_channel_0_idx].st;
    ch0_dest_reg = &MXC_DMA->ch[dma_channel_0_idx].dst;
    ch0_dest_reload_reg = &MXC_DMA->ch[dma_channel_0_idx].dst_rld;
    ch0_count_reg = &MXC_DMA->ch[dma_channel_0_idx].cnt;
    ch0_count_reload_reg = &MXC_DMA->ch[dma_channel_0_idx].cnt_rld;

    ch1_config_reg = &MXC_DMA->ch[dma_channel_1_idx].cfg;
    ch1_status_reg = &MXC_DMA->ch[dma_channel_1_idx].st;
    ch1_dest_reg = &MXC_DMA->ch[dma_channel_1_idx].dst;
    ch1_dest_reload_reg = &MXC_DMA->ch[dma_channel_1_idx].dst_rld;
    ch1_count_reg = &MXC_DMA->ch[dma_channel_1_idx].cnt;
    ch1_count_reload_reg = &MXC_DMA->ch[dma_channel_1_idx].cnt_rld;
    ...
    *ch1_config_reg = 0;
    ...
    // (2) set data destination to the first chunk
    *ch0_dest_reg = (uint32_t)dma_chunks_ch0[0];
    *ch1_dest_reg = (uint32_t)dma_chunks_ch1[0];
    ...
    *ch0_config_reg =
        ...
        MXC_F_DMA_CFG_DSTINC |            // dstinc, destination auto increment
    ...
    *ch0_dest_reload_reg = (uint32_t)dma_chunks_ch0[1];
    *ch0_count_reload_reg = AUDIO_DMA_BUFF_LEN_IN_BYTES;

    *ch1_dest_reload_reg = (uint32_t)dma_chunks_ch1[1];
...
int audio_dma_start()
{
    // Re-initialize all DMA buffer chunks
    for (int i = 0; i < DMA_NUM_STALLS_ALLOWED; i++)
    {
        memset(dma_chunks_ch0[i], 0, AUDIO_DMA_BUFF_LEN_IN_BYTES);
        memset(dma_chunks_ch1[i], 0, AUDIO_DMA_BUFF_LEN_IN_BYTES);
    }
    ...
    MXC_DMA->cn |= (1 << dma_channel_0_idx) | (1 << dma_channel_1_idx);
    ...
    const uint32_t ch0_spi_ctrl0_control_word = (bsp_spi_adc_ch0_data_spi_handle->ctrl0 | MXC_F_SPI_CTRL0_SPI_EN);
    const uint32_t ch1_spi_ctrl0_control_word = (bsp_spi_adc_ch1_data_spi_handle->ctrl0 | MXC_F_SPI_CTRL0_SPI_EN);
...
int audio_dma_stop()
{
    ...
    *ch1_config_reg &= ~(MXC_F_DMA_CFG_RLDEN | MXC_F_DMA_CFG_CHEN);
    ...
    // disable SPI
    bsp_spi_adc_ch0_data_spi_handle->ctrl0 &= ~MXC_F_SPI_CTRL0_SPI_EN;
    bsp_spi_adc_ch1_data_spi_handle->ctrl0 &= ~MXC_F_SPI_CTRL0_SPI_EN;
...
uint8_t *audio_dma_consume_buffer(Audio_Channel_t channel)
{
    ...
    {
        retval = dma_chunks_ch0[blockPtrModulo_ch0];
        blockPtrModulo_ch0 = (blockPtrModulo_ch0 + 1) % DMA_NUM_STALLS_ALLOWED;
        num_buffer_chunks_with_data_to_be_consumed_ch0 -= 1;
    }
    else
    {
        retval = dma_chunks_ch1[blockPtrModulo_ch1];
        blockPtrModulo_ch1 = (blockPtrModulo_ch1 + 1) % DMA_NUM_STALLS_ALLOWED;
...
void dma_ch0_callback(int ch, int reason)
{
    ...
    /* This modulo index counts [0, 1, 2, ..., 9, 0, 1, ...], this represents the chunk number.
     * It is used to look up the next chunk address from the pointer array.
     *
    ...
    *ch0_config_reg |= (MXC_F_DMA_CFG_RLDEN);
    ...
    block_idx_modulo = (block_idx_modulo + 1) % DMA_NUM_STALLS_ALLOWED;
    *ch0_dest_reload_reg = (uint32_t)dma_chunks_ch0[block_idx_modulo];
...
void dma_ch1_callback(int ch, int reason)
{
    ...
    *ch1_config_reg |= (MXC_F_DMA_CFG_RLDEN);
    ...
    block_idx_modulo = (block_idx_modulo + 1) % DMA_NUM_STALLS_ALLOWED;
    *ch1_dest_reload_reg = (uint32_t)dma_chunks_ch1[block_idx_modulo];
