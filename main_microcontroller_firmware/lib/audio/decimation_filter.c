...
static void decimate_16x_iirHB_ch0( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len);

static void decimate_16x_iirHB_ch1( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len);

static void decimate_8x_iirHB_ch0( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len);

static void decimate_8x_iirHB_ch1( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len);

static void decimate_4x_iirHB_ch0( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len);

static void decimate_4x_iirHB_ch1( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len);

static void decimate_2x_iirHB_ch0( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len);

static void decimate_2x_iirHB_ch1( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len);
    ...
    Audio_Channel_t channel)
{
    ...
    case AUDIO_SAMPLE_RATE_192kHz:
        ...
        {
            decimate_2x_iirHB_ch0(src_384kHz, dest, dest_len_in_samps);
        }
        else
        {
            decimate_2x_iirHB_ch1(src_384kHz, dest, dest_len_in_samps);
        }
        break;
    ...
    case AUDIO_SAMPLE_RATE_96kHz:
        ...
        if (channel == AUDIO_CHANNEL_0)
        {
            decimate_4x_iirHB_ch0(src_384kHz, dest, dest_len_in_samps);
        }
        else
        {
            decimate_4x_iirHB_ch1(src_384kHz, dest, dest_len_in_samps);
        }
        break;
    ...
    case AUDIO_SAMPLE_RATE_48kHz:
        ...
        if (channel == AUDIO_CHANNEL_0)
        {
            decimate_8x_iirHB_ch0(src_384kHz, dest, dest_len_in_samps);
        }
        else
        {
            decimate_8x_iirHB_ch1(src_384kHz, dest, dest_len_in_samps);
        }
        break;

    case AUDIO_SAMPLE_RATE_24kHz:
        dest_len_in_samps = num_samps_to_filter / 16;
        if (channel == AUDIO_CHANNEL_0)
        {
            decimate_16x_iirHB_ch0(src_384kHz, dest, dest_len_in_samps);
        }
        else
        {
            decimate_16x_iirHB_ch1(src_384kHz, dest, dest_len_in_samps);
        }
        break;

    case AUDIO_SAMPLE_RATE_384kHz:
    default:
...
void decimate_16x_iirHB_ch0( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len) // note len is the final decimated output length, = input length/16
{
    uint32_t k;
...
void decimate_16x_iirHB_ch1( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len) // note len is the final decimated output length, = input length/16
{
...
void decimate_8x_iirHB_ch0( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
...
void decimate_8x_iirHB_ch1( // takes 1.7ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len) // note len is the final decimated output length, = input length/8
{
...
void decimate_4x_iirHB_ch0( // takes 1.6ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len) // note len is the final decimated output length, = input length/4
{
    uint32_t k;
...
void decimate_4x_iirHB_ch1( // takes 1.6ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len) // note len is the final decimated output length, = input length/4
{
...
void decimate_2x_iirHB_ch0( // takes 3.2ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len) // note len is the final decimated output length, = input length/2
{
    uint32_t k;
...
void decimate_2x_iirHB_ch1( // takes 3.2ms, new design (7/28/24) with 50dB
    q31_t *pSrc,
    q31_t *pDst,
    uint32_t len) // note len is the final decimated output length, = input length/2
{
